...
LIMIT 1;
DROP VIEW IF EXISTS v_student_feedback;
CREATE VIEW v_student_feedback AS
SELECT
...
FROM feedback AS f
INNER JOIN students AS s ON f.student_id = s.id
INNER JOIN courses  AS c ON f.course_id  = c.id;
SELECT
    username,
    course_title,
    comment,
