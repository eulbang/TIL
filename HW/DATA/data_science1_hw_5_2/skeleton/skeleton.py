...
BUS_API_KEY = "---"  # 서울시 버스 교통 데이터 API 키 (빈칸 채우기)
SUBWAY_API_KEY = "---"  # 서울시 지하철 교통 데이터 API 키 (빈칸 채우기)
...
def get_bus_data(date):
    ...
    for row in root.findall('.//row'):
        bus_data.append({
            ...
            '노선번호': row.find('RTE_NM').text if row.find('USE_YMD') is not None else '',  # row에서 'RTE_NM' 태그의 텍스트 추출)
            '정류장명': row.find('SBWY_STNS_NM').text if row.find('USE_YMD') is not None else '',  # row에서 'SBWY_STNS_NM' 태그의 텍스트 추출)
...
def get_subway_data(date):
    ...
    for row in root.findall('.//row'):
        subway_data.append({
            ...
            '역명': row.find('SBWY_STNS_NM').text if row.find('USE_YMD') is not None else '',  # row에서 'SBWY_STNS_NM' 태그의 텍스트 추출)
