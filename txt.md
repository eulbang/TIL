...
## Apache Airflow란?
- Python을 기반의 데이터 파이프라인 자동화 및 스케줄링 도구
...
- Executor -> 태스크 실행 방식(Local, Celery, Kubernetes 등)
...
## DAG 실행
- DAG 실행 방법
  - 자동 실행 : DAG에 설정된 스케줄(Interval, crom 등)에 따라 실행
  - 수동 실행 : Web UI 또는 CLI(Command Line Interface)를 통해 즉시 실행
  - 이벤트 기반 실행 : 특정 트리커(예: API 요청, 파일 업로드 등)로 실행
...
- 선행 단계를 완료한 후 Airflow 웹 UI에서  
  `dags_bash_operator` 파일이 업로드 되었는지 확인

# 퀴즈

## Apache Airflow의 Scheduler가 수행하는 주요 역할이 아닌 것은?

1. DAG 파일을 주기적으로 스캔하고, 새로운 DAG을 감지한다.  
2. DAG 실행을 스케줄링하고 Task의 실행 상태를 관리한다.  
3. DAG 및 Task를 실행하며 결과를 직접 저장한다.  
4. Executor에 실행할 Task를 전달하여 실제 실행을 담당하게 한다.