...
- 반복 실험의 문제점  
  ...
  - 예시 시나리오  
    - 1차 실험: 랜덤포레스트, max_depth=5 → accuracy: 0.87  
    - 2차 실험: 랜덤포레스트, max_depth=10 → accuracy: 0.90  
    - 3차 실험: XGBoost, max_depth=5 → accuracy: 0.93
...
- 실험 정보 구조화
  - 실험 정보를 체계적으로 기록하는 방법
    ...
    - Params: 실험 설정값(하이퍼파라미터)  
      예시: Learning_rate=0.01, max_depth=5
    - Metrics: 성능 지표  
      예시: Accuracy=0.89, auc=0.91
    - Tags: 실험 설명, 날짜 등 메타정보  
      예시: Exp_purpose=baseline_test, author=kim
    - Artifacts: 결과 파일, 시각화, 모델 등  
      예시: 모델 파일, confusion_matrix.png, 로그
